	...
	if(obj.cursor != 'pointer'){
		...
		obj.addEventListener("pressmove", function(evt) {
			if(!gamePause && obj.isDragging){
				...
				// Smoother activation - only 3 pixels movement needed OR 100ms hold time
				var timeSinceStart = Date.now() - obj.dragStartTime;
				if(!obj.hasMoved && ((deltaX > 3 || deltaY > 3) || timeSinceStart > 100)){
					obj.hasMoved = true;
					selectObj(obj);
				}
			}
		...
		obj.addEventListener("pressup", function(evt) {
			if(!gamePause && obj.isDragging){
				...
				if(obj.hasMoved && curItem && curItem.name === obj.name){
					...
					// Check if released near original position (within 100 pixels for easier cancel)
		...
		// Add touch-specific optimization for mobile
