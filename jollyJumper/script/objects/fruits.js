var fruits = function(game){
    ...
    fruits.prototype = {
        ...
        initialFruits: function(){
            // Place initial fruits above existing platforms instead of random scatter
            var state = game.state.getCurrentState();
            var platforms = state.platforms && state.platforms.pltGroup;

            if (!platforms) { return; }

            platforms.forEachAlive(function(plt){
                var fruit = this.fruitsGroup.getFirstDead();
                if (fruit) {
                    // Slightly above the platform
                    fruit.reset(plt.x, plt.y - 40);
                }
            }, this);
        },
        ...
        createFruits: function(elem){
            // Spawn a new fruit above a random existing platform near the camera
            var state = game.state.getCurrentState();
            var platforms = state.platforms && state.platforms.pltGroup;
            if (!platforms) { return; }
            ...
            var candidates = [];
            platforms.forEachAlive(function(plt){
                // consider platforms within some vertical range above the camera
                if (plt.y > game.camera.y - 300 && plt.y < game.camera.y + game.height) {
                    candidates.push(plt);
                }
            }, this);
            ...
            var target = game.rnd.pick(candidates);
            var tmp = this.fruitsGroup.getFirstDead();
            if (tmp) {
                tmp.reset(target.x, target.y - 40);
            }
            return;
        },
