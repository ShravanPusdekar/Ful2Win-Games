<template>
    <div class="container-scene">
        ...
        <canvas :class="{'no-cursor' : !settings.showCursor, 'game-over-blur': died}" ref="canvas" id="canvas"></canvas>
...
function createControls(camera : THREE.Camera) {
    ...
    (canvas.value as HTMLCanvasElement).ontouchmove = (e) => {
        ...
        controlCameraTouch(e, camera);
        sword.move(e);
    };

    (canvas.value as HTMLCanvasElement).ontouchstart = (e) => {
        e.preventDefault();
...
// Take touch event and camera as input and handle controls for the camera
function controlCameraTouch(e : TouchEvent, camera : THREE.Camera) {
    const touch = e.touches[0];
    const delta = new THREE.Vector2();

    if(gameState.mouse.x === -1 && gameState.mouse.y === -1) {
        delta.x = window.innerWidth / 2 - touch.clientX;
        delta.y = window.innerHeight / 2 - touch.clientY;
    }
    else {
        delta.x = gameState.mouse.x - touch.clientX;
        delta.y = gameState.mouse.y - touch.clientY;
    }
...
onMounted(() => {
    ...
    gameState.addEventListener(EVENTS.load, () => {
        loading.value = false;
        // Hide overlay and start countdown automatically after loading
        hideOverlay.value = true;
        setTimeout(() => {
            sceneOverlay.value?.startCountdown();
        }, 500);
    });
