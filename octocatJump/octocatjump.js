...
(function octocatJump($, Crafty) {
    $(document).ready(function documentReady() {
        ...
        function createJoystick() {
            ...
            if($("#octo-joystick").length) return;
            ...
            var $joystick = $("<div id='octo-joystick'></div>").css({
                position: "fixed",
                bottom: "30px",
                left: "50%",
            ...
            var $knob = $("<div id='octo-joystick-knob'></div>").css({
                position: "absolute",
                width: knobSize + "px",
                height: knobSize + "px",
                left: (halfSize - halfKnob) + "px",
                top: (halfSize - halfKnob) + "px",
                borderRadius: "50%",
            ...
            function resetKnob() {
                ...
                joystickActive = false;
                setInputFromDelta(0);
                $knob.css("left", (halfSize - halfKnob) + "px");
            }
            ...
            function handleMove(clientX) {
                ...
                var rect = $joystick[0].getBoundingClientRect();
                var centerX = rect.left + halfSize;
                var dx = clientX - centerX;
                var maxRange = halfSize - halfKnob;

                if(dx < -maxRange) dx = -maxRange;
                if(dx > maxRange) dx = maxRange;

                $knob.css("left", (halfSize - halfKnob + dx) + "px");
                setInputFromDelta(dx);
            }
            ...
            $(window).on("pointermove.octojoystick", function (e) {
                if(!active) return;
                e.preventDefault();
                handleMove(e.clientX);
            });

            $(window).on("pointerup.octojoystick pointercancel.octojoystick", function (e) {
                if(!active) return;
                e.preventDefault();
                handleEnd();
            });
        ...
        function showJoystick(x, y) {
            ...
            var $joystick = $("#octo-joystick");
            if(!$joystick.length) {
                createJoystick();
                $joystick = $("#octo-joystick");
            }

            var css = { display: "block" };

            if(typeof x === "number" && typeof y === "number") {
                var size = $joystick.width();
                var halfSize = size / 2;
                ...
                if(left < 0) left = 0;
                if(top < 0) top = 0;
                if(left + size > winW) left = winW - size;
                if(top + size > winH) top = winH - size;
        ...
        function hideJoystick() {
            ...
            var $joystick = $("#octo-joystick");
            if(!$joystick.length) return;

            joystickActive = false;
            mobileInput.left = false;
            mobileInput.right = false;

            var $knob = $("#octo-joystick-knob");
            if($knob.length) {
                var size = $joystick.width();
                var knobSize = $knob.width();
                var halfSize = size / 2;
                var halfKnob = knobSize / 2;
                $knob.css("left", (halfSize - halfKnob) + "px");
            }
        ...
        Crafty.scene("main", function mainScene() {
            ...
            if(IS_MOBILE) {
                if(Crafty.stage && Crafty.stage.elem) {
                    (function (stageElem) {
                        function onFirstPointerDown(e) {
                            if(isDead) return;
                        ...
                        stageElem.addEventListener("pointerdown", onFirstPointerDown, false);
                        stageElem.addEventListener("touchstart", onFirstPointerDown, false);
                    })(Crafty.stage.elem);
                } else {
                    showJoystick();
                }
