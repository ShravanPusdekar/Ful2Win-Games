...
//      lowLag.play(randFromArray(heartSounds));
  ...
  update: function() {
    ...
    if(gameOver){
      ...
      // Show percentage-based score (same logic as updateStatus)
      var deathPercent = (1 - (-this.totalGameTimer.delta() / this.gameTotalTime)) * 100;
      $('.score').html(Math.round(deathPercent * 10) / 10);
      totalTime.pause();
      this.ekgs.push(this.spawnEntity( EntityFlatEKG, this.getRenderedWidth(), offsetTop ));
      this.ekgs.push(this.spawnEntity( EntityFlatEKG, this.getRenderedWidth(), offsetTop ));
      this.ekgs.push(this.spawnEntity( EntityFlatEKG, this.getRenderedWidth(), offsetTop ));
    ...
    if(this.gameTimer.delta() > 0 || this.gameLevel == -1){
      ...
      if(this.gameLevel == 0){
        ...
        if(!this.winlocation){
          ...
          $('.win').fadeIn();
          $('.totalTime').html(Math.round(totalTime.delta())+' seconds');
          var winPercent = (1 - (-this.totalGameTimer.delta() / this.gameTotalTime)) * 100;
          $('.score').html(Math.round(winPercent * 10) / 10);
          totalTime.pause();
          this.totalGameTimer = new ig.Timer();
