...
function Game(){
    ...
    // Track which containers have been scored already
    this.scoredContainers = new Set();
...
Game.prototype.updateInfoAfterMove = function(fromIdx,toIdx){
    ...
    // Check both containers - score newly completed ones, unmark incomplete ones
    this.checkAndScoreContainer(fromIdx);
    this.checkAndScoreContainer(toIdx);
};

Game.prototype.checkAndScoreContainer = function(containerIdx){
    // Check if this container is complete (full with all same color)
    const container = this.table[containerIdx];
    const isComplete = container.length === H && 
                       arrayEqualItems(container) && 
                       this.hiddenItems[containerIdx] === 0;
    
    if (isComplete && !this.scoredContainers.has(containerIdx)) {
        // Container just became complete - score it!
        this.scoredContainers.add(containerIdx);
        
        // Increment the score
        if (typeof window.incrementGameScore === 'function') {
            window.incrementGameScore();
            console.log('Container ' + containerIdx + ' completed! Score increased.');
        }
    } else if (!isComplete && this.scoredContainers.has(containerIdx)) {
        // Container was previously complete but is no longer (undo scenario)
        // Remove from scored set so it can be scored again if completed later
        // But don't decrease the score
        this.scoredContainers.delete(containerIdx);
        console.log('Container ' + containerIdx + ' no longer complete (possibly undone).');
    }
};
 ...
 Game.prototype.restart = function(){
    ...
    // Reset scored containers when restarting
    this.scoredContainers = new Set();
 };
...
function showGameOutcome(){
    if(game.isSolved()){
        // Show brief success message then start new level
        // (Score already incremented per container completion)
        wonMsg_dialog.show(); // non modal dialog
        
        // Automatically start new level after 1 second
        setTimeout(function() {
            wonMsg_dialog.close();
        ...
        if (statusOfMoves==1) {
            // No more useful moves - GAME OVER
            noMoreUsefulMovesMsg_dialog.show(); // non modal dialog
            
            // Trigger game over after brief display
            setTimeout(function() {
                triggerGameOver();
            }, 1500);
        }
        else if (statusOfMoves==2) {
            // No more moves at all - GAME OVER
...
// Function to trigger game over with blur and postMessage
function triggerGameOver() {
    // Close any open dialogs first
    if (wonMsg_dialog) wonMsg_dialog.close();
    if (noMoreUsefulMovesMsg_dialog) noMoreUsefulMovesMsg_dialog.close();
    if (noMoreMovesMsg_dialog) noMoreMovesMsg_dialog.close();
    
    // Call the main endGame function from the timer system
    // This will handle blur, postMessage, and stopping the timer
    if (typeof window.endGameNow === 'function') {
        window.endGameNow();
    }
}
...
function newBtn_callback(){
    closeGameOutcome();
    newGame();
    // Reset timer and score for new game
    if (typeof window.resetGameTimer === 'function') {
