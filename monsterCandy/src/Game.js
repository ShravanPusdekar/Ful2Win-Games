...
Candy.Game.prototype = {
	create: function(){
		...
		// place lives text a bit further right so it doesn't overlap score
		Candy._livesText = this.add.text(280, 20, "Lives: 3", this._fontStyle);
		this._timerText = this.add.text(Candy.GAME_WIDTH-20, 20, "2:00", this._fontStyle);
		this._timerText.anchor.setTo(1, 0);
	...
	update: function(){
		...
		this._timeRemaining -= this.time.elapsed;
		if (this._timeRemaining < 0) {
			this._timeRemaining = 0;
		}
		var totalSeconds = Math.ceil(this._timeRemaining / 1000);
		var minutes = Math.floor(totalSeconds / 60);
		var seconds = totalSeconds % 60;
		var timeString = minutes + ":" + (seconds < 10 ? "0" + seconds : seconds);
		this._timerText.setText(timeString);
		// if the health of the player drops to 0, the player dies = game over
		if(!this._isGameOver && (!Candy._health || this._timeRemaining === 0)) {
			// show the game over message
			this.add.sprite((Candy.GAME_WIDTH-594)/2, (Candy.GAME_HEIGHT-271)/2, 'game-over');
			// pause the game
			this.game.paused = true;
			this._isGameOver = true;
			// expose final score and notify parent container about game over
			this.game.score = Candy._score;
...
Candy.item = {
	...
	removeCandy: function(candy){
		...
		Candy._health -= 1;
		if (Candy._livesText) {
			Candy._livesText.setText("Lives: " + Candy._health);
		}
	}
};